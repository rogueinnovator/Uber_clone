...
router.post("/register",
    ...
    body("password").isLength({ min: 3 }).withMessage("password must be at least 3 digit long"),
    body("fullname.firstname").isLength({ min: 3 }).withMessage("firstname should be at least 3 char long"),
    body("vehicle.color").isLength({ min: 3 }).withMessage("color must be atleat 3 char long"),
    body("vehicle.plate").isInt({ min: 1 }).withMessage("plate no must be at least 1 number"),
